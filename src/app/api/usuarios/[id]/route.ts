...
export async function PATCH(
  ...
  return handle(async () => {
    ...
    const sql = masterDb()
    const usuario = await UsuariosService.updateUsuario(sql, numId, tenantSlug, parsed.data, user.username)
...
export async function DELETE(
  ...
  return handle(async () => {
    ...
    if (!Number.isInteger(numId) || numId <= 0) throw new ValidationError('Invalid user id')
    const sql = masterDb()
    await UsuariosService.deleteUsuario(sql, numId, tenantSlug, user.username)
